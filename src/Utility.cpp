...
Utility::Utility(string name, int price, int rent) {
	this->ownerID = -1;
	this->price = price;
	this->name = name;
	this->rent = rent;
...
int Utility::ShowBuyMenu(Player& player) {
	cout << player.GetName() << " now you have " << player.GetMoneyInTheWallet() << " Rs." << endl;
	cout << player.GetName() << " You can buy this utility!" << endl;
	cout << "To buy this utility you have to pay : " << GetPrice() << endl;
...
void Utility::ShowSellOrMortgageMenu(Player& player) {
	if (!isMortgaged) {
...
void Utility::DeductRent(Player& player) {
...
int Utility::RelevantFunction(class Player& player, int playerTurn) {
